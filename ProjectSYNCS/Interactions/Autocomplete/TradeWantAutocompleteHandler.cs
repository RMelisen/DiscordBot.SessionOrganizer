        ...
        IServiceProvider services)
    {
        ...
        if (!ulong.TryParse(userOption, out var userId))
        {
            return AutocompletionResult.FromSuccess(ItemCatalog.All.Where(Matches)
                ...
                .Take(MaxSuggestions)
                .Select(i => new AutocompleteResult($"{ItemCatalog.TextEmoji(i)}{ItemCatalog.ClearName(i)}", i.Key)));
        }
        ...
        return AutocompletionResult.FromSuccess(held
            ...
            .Take(MaxSuggestions)
            .Select(x => new AutocompleteResult($"{ItemCatalog.TextEmoji(x.Info!)}{ItemCatalog.ClearName(x.Info!)} ×{x.Row.Quantity}", x.Info!.Key)));
    }
}
